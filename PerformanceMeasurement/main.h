...
public:
    ...
    int init(CONFIG& Config){
        ...
        if(Config.isGenOutFile){
            outStream.open(Config.OutFilePath, std::ifstream::out | std::ios::binary);
            if(!outStream.is_open()){
                outStream.close();
                std::cerr << "ERROR: failed to open output file" << std::endl;
                return 1;
    ...
    int WriteOneTrace(CONFIG& Config){
        ...
        if(Config.isTrace == true){
            ...
            // output raw data
            outStream<<"Time Stamp (s) : " << RelativeTimeStamp << std::endl;
            if(Config.isMeasureEnergy==true){
                outStream << "Power (W) : " << ((float)currPower)/1000.0 << std::endl;
            }
            if(Config.isMeasureGPUUtil==true){
                outStream << "GPUUtil (%) : " << currUtil.gpu << std::endl;
            }
            if(Config.isMeasureSMClk==true){
                outStream  << "SMClk (MHz) : " << currSMClk << std::endl;
            }
            if(Config.isMeasureMemUtil==true){
                outStream  << "MemUtil (%) : " << currUtil.memory << std::endl;
            }
            if(Config.isMeasureMemClk==true){
                outStream << "MemClk (MHz) : " << currMemClk << std::endl;
            }
            outStream << std::endl;
        }
